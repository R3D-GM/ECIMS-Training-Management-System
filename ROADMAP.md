...
## "Every role saved on Consignee, connected via Role Mapper, keyed by username" (added)

Two things added from your latest notes:

1. **Consignee.Code = username.** Every person's Consignee record now
   carries their username in the `Code` field, so his system can find/match
   a person by the same username they log in with - not just by an
   internal Id.

2. **Login now double-checks the sync.** Added `External/UserSyncCoordinator.cs`
   - one shared method (`EnsureUserSyncedAsync`) that does the full
   Consignee -> User -> UserRoleMapper chain. It's called when a user is
   created (as before), AND now also at login (`AccountController.Login`)
   as a safety net: if someone was created before the endpoint was
   configured, or a sync attempt failed, the very next time they log in it
   tries again automatically. It only re-sends what's missing (checks
   `ExternalConsigneeId`/`ExternalUserId` first), so it won't create
   duplicates on every login once it succeeds.

3. **Every role now gets its own Consignee slot on the Voucher**, not just
   the primary actor and the company:
   - Training Session Voucher: Consignee1 = Trainer, Consignee2 = Company,
     Consignee3 = Contact Person, Consignee4 = Created By
   - UAT Project Voucher: Consignee1 = Consultant, Consignee2 = Company,
     Consignee3 = Contact Person, Consignee4 = Project Manager,
     Consignee5 = Created By
   This slot assignment is my best-guess ordering, not something he
   confirmed - worth double-checking with him that Consignee1-5 in that
   order is what he expects, versus a different order or a completely
   different Voucher per role.
