    ...
    public async Task<IActionResult> SaveChecklist(int attemptId, List<ChecklistItemInput> items, string action = "save")
    {
        ...
        if (action == "submit")
        {
            ...
            var project = await _db.UatProjects.Include(p => p.CompanyBranch).ThenInclude(b => b!.Company)
                .Include(p => p.Consultant).Include(p => p.ProjectManager)
                .FirstOrDefaultAsync(p => p.Id == attempt.UatProjectId);
            if (project?.ExternalVoucherId is not null)
            {
                var contactPerson = await _userManager.Users.FirstOrDefaultAsync(u => u.CompanyId == project.CompanyBranch!.CompanyId);
                var dto = ExternalMapper.ToVoucher(project, ConstantCodes.VoucherDefinition_UatSubmitted,
                    project.Consultant?.ExternalConsigneeId, project.CompanyBranch?.Company?.ExternalConsigneeId,
                    contactPerson?.ExternalConsigneeId, project.ProjectManager?.ExternalConsigneeId,
