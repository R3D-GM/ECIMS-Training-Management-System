...
public class UatProjectsController : Controller
{
    ...
    public async Task<IActionResult> Create(UatProject model, int companyId)
    {
        ...
        var voucherDto = ExternalMapper.ToVoucher(model, ConstantCodes.VoucherType_UatProject, consultant?.ExternalConsigneeId, branch.Company?.ExternalConsigneeId, user?.ExternalUserId ?? 0, projectManager?.ExternalConsigneeId, user?.ExternalConsigneeId);
