...
public class TrainingSessionsController : Controller
{
    ...
    public async Task<IActionResult> AddTrainee(int trainingSessionId, string name, string? position, string? phone)
    {
        if (!string.IsNullOrWhiteSpace(name))
        {
            ...
            // Re-push the voucher so this trainee lands in a Consignee slot
            // on the training record too, not just as a standalone Consignee.
            var session = await _db.TrainingSessions.FindAsync(trainingSessionId);
            if (session is not null)
            {
                await SyncTrainingVoucher(session, isNew: false);
            }
        }
    ...
    // Notifies the assigned trainer and the company's contact person whenever
    // a session is scheduled or changed for them.
    ...
    private async Task SyncTrainingVoucher(TrainingSession model, bool isNew)
    {
        ...
        var traineeConsigneeIds = await _db.Trainees
            .Where(t => t.TrainingSessionId == model.Id && t.ExternalConsigneeId != null)
            .OrderBy(t => t.Id)
            .Select(t => t.ExternalConsigneeId!.Value)
            .ToListAsync();
        var currentUser = await _userManager.GetUserAsync(User);

        var dto = ExternalMapper.ToVoucher(model, trainer?.ExternalConsigneeId, company?.ExternalConsigneeId, currentUser?.ExternalUserId ?? 0, traineeConsigneeIds);
