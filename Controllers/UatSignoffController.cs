...
public class UatSignoffController : Controller
{
    ...
    public async Task<IActionResult> Sign(int attemptId, string? signatureDataUrl)
    {
        ...
        {
            var contactPerson = await _userManager.Users.FirstOrDefaultAsync(u => u.CompanyId == attempt.UatProject.CompanyBranch!.CompanyId);
            var dto = ExternalMapper.ToVoucher(attempt.UatProject, ConstantCodes.VoucherDefinition_UatSigned,
                attempt.UatProject.Consultant?.ExternalConsigneeId, attempt.UatProject.CompanyBranch?.Company?.ExternalConsigneeId,
                contactPerson?.ExternalConsigneeId, attempt.UatProject.ProjectManager?.ExternalConsigneeId,
